...
const getById = async (id) => {
  const item = await prisma.agradecimiento.findUnique({ where: { id }, include });
...
const create = async (data) => {
  try {
    if (data.referenciaId) {
      const referencia = await prisma.referencia.findUnique({ where: { id: data.referenciaId } });
      if (!referencia) throw { status: 400, message: "La referencia no existe." };

      const normalize = (s) => s.normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().toLowerCase();

      if (data.emisorId  !== referencia.receptorId) throw { status: 400, message: "El emisorId no coincide con la referencia." };
      if (data.receptorId !== referencia.emisorId)  throw { status: 400, message: "El receptorId no coincide con la referencia." };
      if (normalize(data.nombreContacto) !== normalize(referencia.nombreContacto))
        throw { status: 400, message: "El nombre de contacto no coincide con la referencia." };
    }
    ...
    if (err.code === "P2003") throw { status: 400, message: "Error de integridad referencial." };
    throw err;
  }
};
...
const remove = async (id, userId) => {
  const item = await prisma.agradecimiento.findFirst({ where: { id, emisorId: userId } });
  if (!item) throw { status: 404, message: "Agradecimiento no encontrado." };
